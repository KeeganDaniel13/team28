...
namespace CiroService
{
    ...
    public class Service1 : IService1
    {
        ...
        public string stockTake(stockTakeInfo stockTake)
        {
            ...
            {
                //update a table
                if(stockInWarehouse.inTransit.Equals("N"))
                {
                    //upadte package in same warehouse
                    stockInWarehouse.lastdatechecked = DateTime.Now;
                    stockInWarehouse.numberofchecks += 1;
                    stockTable.updateRecord(stockInWarehouse.warehousestock_id, stockInWarehouse);
                }
                else
                {
                    //update package if received in different warehouse
                    stockInWarehouse.lastdatechecked = DateTime.Now;
                    stockInWarehouse.numberofchecks += 1;
                    stockInWarehouse.warehouse_id_warehousestock = warehouses.First(c => c.warehouse_location.Equals(stockTake.location)).warehouse_id;
                    stockTable.updateRecord(stockInWarehouse.warehousestock_id, stockInWarehouse);
                }
            }
        ...
        //adds an incident report to the specific product -either with an image or not
        //Test With Andriod Streaming
        public void addIncident(jsonIncident newIncident)
        {
            incidentController incidentTable = new incidentController();
            IEnumerable<productincident> incidents = incidentTable.getTable();
            var stream="";
            if (stream == null)
            {
                incidentTable.addRecord(new productincident { productincident_product_id = newIncident.productID, productincident_description = newIncident.description, productincident_id = incidents.Count(), productincident_image = "N/A" });
                return;
            }
            else
            {
                FileStream newImage = null;
               // using (newImage = new FileStream)
                {

                }
                incidentTable.addRecord(new productincident { productincident_product_id = newIncident.productID, productincident_description = newIncident.description, productincident_id = incidents.Count(), productincident_image = "/images/" + newIncident.image });
                return;
            }
        }
